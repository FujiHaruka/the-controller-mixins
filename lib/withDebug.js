...
const getAllPropertyDescriptors = require('./helpers/getAllPropertyDescriptors')
  ...
  class WithDebug extends Class {
    constructor (...args) {
      ...
      unlessProduction(() => {
        ...
        const instanceMethodNames = Object.keys(descriptors)
          .filter((name) => !['constructor', 'toString', 'toJSON', 'then', 'catch', 'finally', 'valueOf'].includes(name))
          .filter((name) => ![/^controllerWill/, /^controllerDid/, /^controllerMethod/].some((pattern) => pattern.test(name)))
